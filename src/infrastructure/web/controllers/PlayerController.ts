...
import { InvalidSeasonException, InvalidYearException, PlayerAlreadyDeletedException, PlayerAlreadyExistsException, PlayerNotDeletedException, PlayerNotFoundException } from '../../../domain/exceptions/PlayerExceptions.js';
...
export class PlayerController {
   ...
   *                 message:
   *                   type: string
   *                   example: Player is already deleted
   *       500:
   *         description: Internal Server Error
   *         content:
   *           application/json:
  ...
  async deletePlayer(req: AuthRequest, res: Response) {
    try {
      ...
      if (error instanceof PlayerAlreadyDeletedException) {
        return res.status(409).json(
          ApiResponseBuilder.error(error.message)
        );
      }
   ...
   *   post:
   *     summary: Restore deleted player
   *     tags: [Players]
   *     security:
   *       - bearerAuth: []
   *     parameters:
   ...
   *                 message:
   *                   type: string
   *                   example: Player restored successfully
   *                 data:
   *                   type: string
   *                   example: null
   *       400:
   ...
   *                   example: Internal server error
   */
  async restorePlayer(req: AuthRequest, res: Response) {
    try {
      const id = req.params.id;
      if (!id || typeof id !== 'string') {
        throw new MissingRequiredUserFieldsException();
      ...
      await playerService.restore(id);

      res.status(200).json(
        ApiResponseBuilder.success(
          null,
          'Player restored successfully',
        )
      );
    } catch (error: any) {
      if (error instanceof MissingRequiredUserFieldsException) {
      ...
      if (error instanceof PlayerNotDeletedException) {
        return res.status(409).json(
          ApiResponseBuilder.error(error.message)
        );
      }
