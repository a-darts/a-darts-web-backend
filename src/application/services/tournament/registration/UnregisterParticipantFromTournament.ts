...
export class UnregisterParticipantFromTournament {
    ...
    public async execute(request: UnregisterParticipantInTournamentRequestDTO): Promise<void> {
        ...
        const registeredParticipant = await this.registeredParticipantRepository.findByTournamentIdAndPlayerId(
            request.id,
            request.playerId,
        );
        ...
        tournament.unregisterParticipant(request.playerId);
